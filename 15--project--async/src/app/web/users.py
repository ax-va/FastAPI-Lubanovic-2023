...
async def create_access_token(
    ...
    is_verified: bool = await service.verify_credentials(
        db_session,
        form_data.username,
        form_data.password
    )
...
async def grant_admin(
    ...
    try:
        user_response: UserResponse = await service.set_admin(db_session, user_id, True)
...
async def revoke_admin(
    ...
    try:
        user_response: UserResponse = await service.set_admin(db_session, user_id, False)
...
async def get(
    ...
    if user_id is not None:
        user_response: UserResponse | None = await service.get_by_id(db_session, user_id)
        ...
        user_response: UserResponse | None = await service.get_by_username(db_session, username)
        ...
        user_responses: list[UserResponse] = await service.get_all(db_session)
...
async def create(
    ...
    try:
        user_response: UserResponse = await service.create(db_session, user_request)
...
async def replace(
    ...
    try:
        user_response: UserResponse = await service.replace(db_session, user_id, user_request)
...
async def soft_delete(
    ...
    try:
        soft_deleted: UserResponse = await service.soft_delete(db_session, user_id)
