...
@pytest.fixture
def sqlite_memory_conn(monkeypatch: pytest.MonkeyPatch) -> Generator[Connection, None, None]:
...
@pytest.fixture
def creatures_sqlite_memory_db(
    sqlite_memory_conn: Connection,
    ...
    monkeypatch.setattr(db, "conn", sqlite_memory_conn)

    sqlite_memory_conn.execute(
    ...
    for sample_request in [yeti_request, bigfoot_request]:
        sqlite_memory_conn.execute(insert_query, sample_request.model_dump())

    sqlite_memory_conn.commit()

    return sqlite_memory_conn


@pytest.fixture
def explorers_sqlite_memory_db(
    sqlite_memory_conn: Connection,
    monkeypatch: pytest.MonkeyPatch,
) -> Connection:
    # Monkeypatch replaces a real object with another real object.
